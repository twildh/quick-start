...
export const refreshBookmarks = (): ThunkActionT => async (dispatch): Promise<void> => {
...
export const createBookmark = (
	...
	url: string,
	index?: number,
): ThunkActionT => async (dispatch): Promise<void> => {
...
export const deleteBookmark = (id: string): ThunkActionT => async (dispatch): Promise<void> => {
	await browser.bookmarks.remove(id);
	dispatch(refreshBookmarks());
};

export const deleteFolder = (id: string): ThunkActionT => async (dispatch): Promise<void> => {
	await browser.bookmarks.removeTree(id);
	dispatch(refreshBookmarks());
};

export const updateBookmark = (id: string, title?: string, url?: string): ThunkActionT => async (
	dispatch,
): Promise<void> => {
	if (!title && !url) {
		throw Error("Error updating bookmark: One of `title` and `url` parameters must be provided");
	}
	await browser.bookmarks.update(id, { title, url });
	dispatch(refreshBookmarks());
};

export const updateFolder = (id: string, title: string): ThunkActionT => updateBookmark(id, title);

export const moveNodeToFolder = (id: string, parentId: string): ThunkActionT => async (
	dispatch,
): Promise<void> => {
...
export const moveNodeInFolder = (id: string, index: number): ThunkActionT => async (
	dispatch,
	getState,
): Promise<void> => {
